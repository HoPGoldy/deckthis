...
**deckthis** 是一个基于 Web 的演示工具。每张幻灯片是一个独立的 HTML 文件，由 deckthis dev server 托管并以 iframe 形式组合成演示文稿。

## 核心概念

- **slide** — 普通 HTML 文件（如 `01.html`、`02.html`），文件名以 `_` 开头的会被忽略
- **deck folder** — 存放所有 slide HTML 的目录，即 `deckthis <folder>` 所指向的目录
...
```bash
deckthis <folder>              # 启动 dev server（默认端口 39200）
deckthis <folder> --port 3000  # 指定端口
deckthis demo list             # 列出内置 demo
deckthis demo <name>           # 复制 demo 到当前目录
deckthis skill                 # 复制本 skill 到指定目录
```
...
```
deckthis-cli    ← CLI 工具，包含 dev server、config loader、子命令
deckthis-core   ← 浏览器端 SlideDeck 运行时（打包为 IIFE 注入页面）
```
