...
describe("dev-server integration", () => {
  ...
  beforeAll(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "deckthis-srv-"));
  ...
  it("GET / returns 200 with shell HTML", async () => {
    ...
    expect(text).toContain("/__deckthis/ppt-wrapper.iife.js");
  });

  it("GET /__deckthis/config returns correct JSON", async () => {
    const res = await fetch(`${base}/__deckthis/config`);
    expect(res.status).toBe(200);
  ...
  it("GET /__deckthis/ppt-wrapper.iife.js returns 404 when vendor file absent", async () => {
    const res = await fetch(`${base}/__deckthis/ppt-wrapper.iife.js`);
...
describe("dev-server assets support", () => {
  ...
  beforeAll(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "deckthis-assets-"));
    assetDir = await fs.mkdtemp(path.join(os.tmpdir(), "deckthis-assetfiles-"));
    secondaryAssetDir = await fs.mkdtemp(path.join(os.tmpdir(), "deckthis-assetfiles-2-"));
    await fs.writeFile(path.join(tmpDir, "01.html"), "<h1>Slide 1</h1>");
...
describe("dev-server beforeEach hook", () => {
  ...
  beforeAll(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "deckthis-before-"));
