...
演示 `deckthis.config.ts` 配置文件的全部能力：自定义幻灯片顺序、全局样式注入、overlay / underlay。

## 使用方法

```bash
deckthis demo with-config   # 复制到当前目录
cd with-config
deckthis .
```
...
```
with-config/
  deckthis.config.ts # 配置文件，使用 defineConfig 导出
...
`getDeckDir()` 由 deckthis 在加载配置时注入，返回当前 deck 目录的绝对路径，
