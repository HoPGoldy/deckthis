...
describe("SlideDeck", () => {
  ...
  describe("collectSlideData", () => {
    ...
    it("reads a single deckthis:title meta", () => {
      const iframe = makeIframeWithMeta({ "deckthis:title": "Hello" });
      expect(collectSlideData(iframe)).toEqual({ title: "Hello" });
    });

    it("reads multiple deckthis:* metas", () => {
      const iframe = makeIframeWithMeta({ "deckthis:title": "My Title", "deckthis:section": "intro" });
      expect(collectSlideData(iframe)).toEqual({ title: "My Title", section: "intro" });
    });

    it("falls back to <title> when deckthis:title is absent", () => {
      const iframe = document.createElement("iframe");
      container.appendChild(iframe);
    ...
    it("does not override deckthis:title with <title> fallback", () => {
      const iframe = makeIframeWithMeta({ "deckthis:title": "Meta Title" });
      iframe.contentDocument!.title = "Page Title";
      expect(collectSlideData(iframe)).toEqual({ title: "Meta Title" });
    });

    it("ignores non-deckthis meta tags", () => {
      const iframe = makeIframeWithMeta({ description: "some desc", "deckthis:title": "T" });
  ...
  describe("slide-data broadcast", () => {
    it("sends deckthis:slide-change to overlay contentWindow on goto()", () => {
      ...
      expect(postMessageSpy).toHaveBeenCalledWith(
        expect.objectContaining({ type: "deckthis:slide-change", current: 2, total: 2 }),
        "*",
      );
      deck.destroy();
    });

    it("sends deckthis:slide-change to underlay contentWindow on goto()", () => {
      const deck = SlideDeck({
        slides: ["/a.html", "/b.html"],
        el: container,
    ...
    it("message includes title collected from slide meta", () => {
      ...
      // Write a deckthis:title meta into slide 1's document before navigating
      const slideEl = container.querySelectorAll<HTMLIFrameElement>(".sd-slide")[1];
      const meta = slideEl.contentDocument!.createElement("meta");
      meta.setAttribute("name", "deckthis:title");
      ...
      expect(postMessageSpy).toHaveBeenCalledWith(
        expect.objectContaining({ type: "deckthis:slide-change", current: 2, total: 2, title: "Slide Two" }),
        "*",
      );
      deck.destroy();
    });
    ...
    it("defers broadcast until load when slide readyState is not complete", () => {
      ...
      expect(slide2CallsAfter[0][0]).toMatchObject({ type: "deckthis:slide-change", current: 3, total: 3 });
      deck.destroy();
    });
  });
});
