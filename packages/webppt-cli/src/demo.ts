...
export async function copyDemo(demosDir: string, name: string, targetBase: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`Demo "${name}" not found. Run \`webppt demo list\` to see available demos.`);
  }
  ...
  const demoCmd = new Command("demo")
    ...
    .action(async (name: string) => {
      ...
      try {
        ...
        console.log(`[webppt] Demo "${name}" copied to ./${name}`);
        console.log(`[webppt] Run it with:`);
        console.log(`  cd ${name} && webppt .`);
      } catch (err: unknown) {
        console.error(`[webppt] ${(err as Error).message}`);
        process.exit(1);
      }
    });
