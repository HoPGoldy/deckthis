...
describe("listDemos", () => {
  ...
  beforeAll(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "webppt-list-"));
...
describe("copyDemo", () => {
  ...
  beforeAll(async () => {
    demosDir = await fs.mkdtemp(path.join(os.tmpdir(), "webppt-demos-"));
    targetDir = await fs.mkdtemp(path.join(os.tmpdir(), "webppt-target-"));
