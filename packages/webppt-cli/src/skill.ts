...
export async function copySkill(skillsDir: string, targetBase: string): Promise<void> {
  const src = path.join(skillsDir, "webppt");
  const dst = path.join(targetBase, "webppt");

  try {
    await fs.access(dst);
    throw new Error(`Directory "webppt" already exists in "${targetBase}".`);
  } catch (err: unknown) {
...
export async function registerSkillCommand(program: import("commander").Command): Promise<void> {
  program
    .command("skill")
    .description("Copy the webppt AI coding skill to a local skills directory")
    .action(async () => {
      ...
      try {
        ...
        console.log(`[webppt] Skill copied to ${path.join(targetSubdir, "webppt")}/`);
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[webppt] ${msg}`);
        process.exit(1);
      }
    });
}
